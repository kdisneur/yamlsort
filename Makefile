...
compile:
	...
	@GOOS=$(OS) GOARCH=$(ARCH) go build $(BUILD_OPTIONS) \
		-ldflags \
		"-X github.com/kdisneur/yamlsort.versionNumber=$$(if [ "$(GIT_TAG)" = "" ]; then echo "unknown"; else echo "$(GIT_TAG)"; fi) \
			 -X github.com/kdisneur/yamlsort/internal.gitBranch=$(GIT_BRANCH) \
			 -X github.com/kdisneur/yamlsort/internal.gitCommit=$(GIT_COMMIT) \
			 -X github.com/kdisneur/yamlsort/internal.gitState=$(GIT_STATE) \
			 -X github.com/kdisneur/yamlsort/internal.buildDate=$(shell date -u +'%Y-%m-%dT%H:%M:%SZ')" \
